...
from bot.keyboards.inline import get_cancel_registration_keyboard, get_tariffs_keyboard
...
async def cancel_registration(event: Message | CallbackQuery, state: FSMContext):
    ...
    text = "❌ Регистрация отменена. Вы можете начать её в любой момент через главное меню или команду /register."
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if not phone:
        await message.answer(
            "⚠️ Не удалось распознать номер телефона. Пожалуйста, воспользуйтесь кнопкой ниже "
            "или введите номер в международном формате (например, <code>+79991234567</code>):",
...
async def process_email(message: Message, state: FSMContext):
    ...
    if not EMAIL_REGEX.match(email):
        await message.answer(
            ...
            "<i>Например: ivanova@gmail.com</i>",
            parse_mode="HTML"
        )
        return
